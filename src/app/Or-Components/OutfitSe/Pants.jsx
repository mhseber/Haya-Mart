...
import PantsMo from "../ProductModalPage/PantsMo";

const Pants = () => {
  const [items, setItems] = useState([]);
  const [selectedItem, setSelectedItem] = useState(null);
  useEffect(() => {
    // fetch("http://localhost:3000/api/items/pants")
    fetch("ProductData/pants.json")
      .then((res) => {
  ...
  return (
    <div className="px-6 py-6">
      ...
      <section className=" gap-3 my-8 pl-5">
        <h2 className="text-3xl font-bold text-blue-200">Pants</h2>
      </section>
      ...
      {selectedItem && (
        <PantsMo item={selectedItem} onClose={handleCloseModal} />
      )}
